...
- **Multi-tenant / per-business connections are unsolved — this is the single biggest open question in this plugin.** `base_service_url` is currently hardcoded to one dev Zendesk account (`kizen-79102.zendesk.com`); every business that installs this plugin as-is would see *that* account's tickets, not their own. Two candidate mechanisms were tested for making `services[]` fields vary per business:
  ...
  - **`additional_service_urls`/`full_domain`/`sub_domain_regex_validation` — tested thoroughly, confirmed real, but solves only HOST ROUTING, not OAuth TOKEN issuance.** A service can declare `additional_service_urls` (alternate hosts) and `sub_domain_regex_validation` (a regex matched against the bare subdomain *label*, not the full host string — e.g. `"^[a-zA-Z0-9-]+$"`); a script can then override the destination per-call with a `full_domain` query param, and this genuinely works — confirmed reaching real, previously-unlisted Zendesk hosts, not silently ignored. Combined with a second finding — Python Code Steps CAN read setup_assistant Configuration values, just not via any Python attribute (`kizen.config`/`kizen.business_config`/bare `config` are all confirmed absent) but via a real HTTP call, `kizen.api.get("/external-integrations/business-plugin-apps/<plugin_api_name>")` → `response.json()["config"]["__kizen_clean_config"]["<field_key>"]` (root URL already bakes in `/api`, don't prepend it) — a script can read a business's own configured `zendesk_subdomain` and build `full_domain` from it dynamically, entirely from script.py, no manifest templating needed. **This is a real, proven mechanism for per-business host routing.** But it does NOT solve the actual multi-tenant blocker: Zendesk OAuth tokens are subdomain/account-scoped, so a token minted by one Zendesk OAuth client can only authenticate calls to that same account regardless of what `full_domain` points at — the connect/authorize flow (`authorize_url`/`token_url`/`client_id`) still resolves statically from the deployed manifest, the same layer that already defeated `{{fieldKey}}` templating for `base_service_url`. Separately confirmed `base_service_url` itself cannot be made dynamic on its own — neither a literal `*` wildcard nor leaving it as a bare root domain (`"zendesk.com"`, with or without scheme) produces a working default host; each either gets rejected outright, crashes on a missing URL scheme, or (once syntactically valid) just hits Zendesk's real marketing site and 301-redirects, since a bare root domain isn't an account-specific API endpoint. **Net conclusion: per-business host routing is solved; per-business OAuth token issuance is not — that's the one open question left, and it needs a direct answer from the platform team (or whoever showed us the `business-plugin-apps` endpoint) on whether the connect/authorize flow has any analogous per-business resolution mechanism.**
