...
**Still open: `client_id`/`client_secret` are static, shared across every business.** This plugin currently only actually works end-to-end for the one Zendesk account whose OAuth client these credentials belong to (the dev/test account used throughout development) — a genuinely different business's Zendesk account would reject this `client_id` outright (Zendesk OAuth clients are registered per-account), even though `authorize_url`/`token_url` now correctly point at that business's own subdomain. The same `{{secret.<key>}}` pattern that solved `authorize_url`/`token_url` should extend to `client_id`/`client_secret`, but this is untested — don't consider multi-tenancy complete until it is.
